...
const pending = ref(true);  // Estado de carga
const error = ref(null);  // Para capturar errores
const router = useRouter();  // Para redirigir al detalle de la película

// Función para obtener las películas desde una API
const fetchMovies = async () => {
  try {
    const response = await fetch(`${config.public.apiBase}/movies`);
    if (response.ok) {
      movies.value = await response.json();  // Guardar películas
    } else {
    ...
    pending.value = false;  // Termina la carga
  }
};
